...
Source Name: VwSchemaObjectMapper.java
 ...
 */
public class VwSchemaObjectMapper
{
  private String m_strDbConfigProps;
   ...
   * @param nGenType
   */
  public VwSchemaObjectMapper( VwSqlMappingSpec mappingSpec, boolean fNoModifyDVOs, int nGenType ) throws Exception
  {
   ...
   * @throws Exception
   */
  public void parse( URL urlFileName,  boolean fGenDAO, int nGenType, boolean fOverWriteXsm, String strIncludes, String strDbConfigProps  ) throws Exception
  {

    setCodeGenFlags( nGenType );

    m_strDbConfigProps = strDbConfigProps;
  ...
  private boolean setup() throws Exception
  {
    ...
    if ( m_strSchema == null )
    {
       m_strSchema = m_db.getDbMgr().getUserID();     // The default
    }
  ...
  private void setupDatabase() throws Exception
  {
    ...
    // *** Log into database
    m_logger.info( null, "Connecting to datasource driver " + m_strAuthor + " using URLID " + strDriverUrlId );
    
    VwDbMgr dbMgr = new VwDbMgr( strDriverName, strDriverUrlId, m_logger );
    m_db = dbMgr.login();
  ...
  public static void main( String[] astrArgs )
  {
    try
    {
      ...
      if ( astrArgs.length > 1  )
      {
        ...
        {
          if ( astrArgs[ x ].equals( "-p"))
          {
            strMappingProperties = astrArgs[++x];
          }
          else
          if ( astrArgs[ x ].equals( "-x"))
          {
            fDAOXMLOnly = true;
            nGenType = VwSchemaObjectMapper.GEN_XML_ONLY;
          }
          else
          if ( astrArgs[ x ].equals( "-d"))
          {
            fDVOOnly = true;
            nGenType = VwSchemaObjectMapper.GEN_DVO_ONLY;
          }
          else
      ...
      dbcg.parse( urlSpecDoc,  fGenDAO,  nGenType, fOverwriteXsm, strIncludes, strMappingProperties );
  ...
  private static void showArgs()
  {
    try
    {
      VwLogger.getInstance().info( null, "Format: VwSchemaObjectMapper <input m_specification document> [-d Generate Java DVO objects only" +
        ...
        "\n-p Mapping properties path i.e ${user.home}/myProject/properties/schemaMapper.properties");
    }
    catch( Exception ex )
    {
      ex.printStackTrace();
...
} // end class VwSchemaObjectMapper{}


// *** End of VwSchemaObjectMapper.java ***
